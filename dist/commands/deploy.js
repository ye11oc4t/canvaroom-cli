...
function forkAndClone(slug, username) {
    const cloneDir = path.resolve(process.cwd(), slug);
    // fork 시도
    const s1 = ora(`${TEMPLATE_REPO} fork 중...`).start();
    let forked = false;
    try {
        execSync(`gh repo fork ${TEMPLATE_REPO} --clone=false --fork-name ${slug}`, { stdio: 'pipe' });
    ...
    if (!forked) {
        ...
        try {
            execSync(`gh repo create ${username}/${slug} --template ${TEMPLATE_REPO} --public`, { stdio: 'pipe' });
...
function injectMeta(cloneDir, encoded, slug, platform) {
    const meta = {
        encoded,
        slug,
        platform,
...
function commitAndPush(cloneDir, title) {
...
function deployVercel(cloneDir, slug, username, prod) {
    ensureVercelCli();
    // 1. 로그인 확인
    ...
    // 2. vercel.json 생성
    fs.writeJSONSync(path.join(cloneDir, 'vercel.json'), { framework: 'nextjs', buildCommand: 'npm run build', installCommand: 'npm install' }, { spaces: 2 });
    // 3. GitHub repo 연결
    ...
    if (connectResult.stdout)
        process.stdout.write(connectResult.stdout);
    if (connectResult.stderr)
        process.stderr.write(connectResult.stderr);
...
function deployGH(cloneDir, username, slug) {
    const s = ora('GitHub Pages 배포 중...').start();
    try {
        fs.writeFileSync(path.join(cloneDir, 'next.config.ts'), `import type { NextConfig } from 'next';
...
`);
        ...
        try {
            execSync(`gh api repos/${username}/${slug}/pages --method POST -f source[branch]=gh-pages -f source[path]=/`, { stdio: 'pipe' });
        }
        catch {
            // 이미 활성화된 경우 무시
        }
...
export async function cmdDeploy(options) {
    ...
    const slugInput = await ask(chalk.white(`  배포 이름 입력 (엔터시 "${defaultSlug}"): `));
